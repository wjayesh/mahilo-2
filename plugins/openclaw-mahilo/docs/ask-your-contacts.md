...
2. Run `/mahilo setup`. If this runtime is new, include both a username and a one-time invite token so Mahilo can bootstrap the identity and store the issued key locally.
3. Run `mahilo status`.
