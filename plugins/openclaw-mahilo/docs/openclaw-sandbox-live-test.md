...
- `plugins/openclaw-mahilo/scripts/dual-sandbox-policy-scenarios.ts`
...
After `dual-sandbox-policy-scenarios.ts` runs, the harness writes `artifacts/provisioning/policy-summary.json` with the exact `S2-allow`, `S3-ask`, `S4-deny`, and `S5-missing-llm-key` selector/message mapping, the seeded policy IDs, the sender LLM defaults used for degraded-review validation, and the note that no manual dashboard work is required before the live proof.
